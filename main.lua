-- FPS BOOST + AIM ASSIST COMPACTO + ESP MM2 + AUTO PEGAR ARMA
...
local function isDeco(o)
    for _,n in ipairs(nomesDeco) do if o.Name:lower():find(n) then return true end end
    return false
end
local function opt(o)
    ...
    elseif o:IsA("ParticleEmitter") or o:IsA("Trail") or o:IsA("Fire") or o:IsA("Smoke") or o:IsA("Sparkles") then o:Destroy()
    elseif (o:IsA("Model") or o:IsA("Folder")) and isDeco(o) then o:Destroy() end
end
...
-- AUTO PEGAR ARMA DO XERIFE
local function pegarArmaRapidinho(arma)
    if not LP.Character then return end
    local hrp=LP.Character:FindFirstChild("HumanoidRootPart")
    local humanoid=LP.Character:FindFirstChild("Humanoid")
    if not hrp or not humanoid then return end
    local velOriginal=humanoid.WalkSpeed
    humanoid.WalkSpeed=100 -- super rápido

    task.spawn(function()
        while (hrp.Position - arma.Position).Magnitude > 3 do
            local dir=(arma.Position - hrp.Position).Unit
...
-- Detecta armas caídas do xerife
W.DescendantAdded:Connect(function(obj)
    if obj:IsA("Tool") and obj.Name:lower():find("gun") then
        -- Espera o objeto ter posição no mundo
        task.wait(0.1)
        pegarArmaRapidinho(obj)
    end
end)
