...
local function opt(o)
    ...
    if o:IsA("BasePart") then
        o.Material=Enum.Material.Plastic;o.Reflectance=0;o.CastShadow=false
    ...
    or o:IsA("Smoke") or o:IsA("Sparkles") then o.Enabled=false;o:Destroy()
...
lbl.Size=UDim2.fromScale(0.1,0.05);lbl.Position=UDim2.new(0,10,0,10)
lbl.BackgroundTransparency=1;lbl.TextColor3=Color3.fromRGB(0,255,0)
lbl.Font=Enum.Font.SourceSansBold;lbl.TextSize=18
local c,lt=0,tick()
R.RenderStepped:Connect(function()
    c+=1
    if tick()-lt>=1 then lbl.Text="FPS: "..c;c=0;lt=tick() end
end)

-- IDENTIFICAR ROLE PELO TEAM
local function getRole(player)
    if not player.Team then return "Inocente" end
    local t=player.Team.Name:lower()
    if t:find("xerife") or t:find("sheriff") then
        return "Xerife"
    elseif t:find("assassino") or t:find("murder") or t:find("killer") then
        return "Assassino"
    end
    return "Inocente"
end
            ...
            if head then
                ...
                if onscreen then
                    ...
                    if dist<closest then
                        ...
                        -- role disponível aqui: getRole(p)
                    end
                end
            end
        end
